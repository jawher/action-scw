...
* `{a}` is the action single version number: there may be multiple versions of this action for a single CLI version, e.g. `v2.5.4` uses the same scw CLI `2.4.4` as before, but adds support for the newly added region parameter.
